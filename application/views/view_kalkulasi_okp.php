...
<html lang="en">
    ...
    <div class="kalkulasi-content"> <!-- start main content -->
        ...
        <div class="fordatagrid"> <!-- start span11 -->
        	<?php 
        		if(!empty($l_okupansi) && !empty($jml_okupansi) && !empty($pv_okupansi)) {	?>
	        <table class="table table-stripped table-bordered table-condensed dtgrid">
	        	<thead>
	        		<th>Okupansi</th>
	        		<?php 
	        			foreach ($l_okupansi as $key => $value) { ?>
	        			<th><?php echo $key; ?></th>
					<?php	        				
	        			}
	        		 ?>
	        	...
	        	<tbody>
	        		<?php 
	        			foreach ($l_okupansi as $key1 => $value1) { 
	        				echo "<tr>"; ?>
	        				<?php
									echo "<td style='font-weight:bold;'>".$key1."</td>";
	        				foreach ($l_okupansi as $key2 => $value2) { ?>
	        				<td><!-- <?php echo $key2."/".$key1; ?> -->
								<?php echo number_format(($value2/$value1),2); ?>
							</td>
							<?php }
								echo "<td>".number_format($pv_okupansi[$key1],4)."</td></tr>";
								$total = $total + $pv_okupansi[$key1];
							}
 							?>
 							<tr class="success">
							<td style='font-weight:bold;'>Jumlah</td>
						<?php 
						foreach ($jml_okupansi as $key => $value) {
	        ...
	        <a href="<?php echo base_url(); ?>dashboard/kalkulasi/3" class="btn btn-inverse" style="float:right;">next</a>
	        <p style="clear:both;" ></p>
						<?php
					} else { echo "<span style='color:white'>insert needed value to start calculating...</span>"; }
						?>	
						<?php error_reporting(0); echo $n; ?>
        </div><!-- end span11 -->
