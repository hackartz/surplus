...
<html lang="en">
    ...
    <div class="kalkulasi-content"> <!-- start main content -->
        ...
        <div class="fordatagrid"> <!-- start span11 -->
        	<?php 
        		if(!empty($l_keba) && !empty($jml_keba) && !empty($pv_keba)) {	?>
	        <table class="table table-stripped table-bordered table-condensed dtgrid">
	        	<thead>
	        		<th>Keb.Air</th>
	        		<?php 
	        			foreach ($l_keba as $key => $value) { ?>
	        			<th><?php echo $key; ?></th>
					<?php	        				
	        			}
	        		 ?>
	        	...
	        	<tbody>
	        		<?php 
	        			foreach ($l_keba as $key1 => $value1) { 
	        				echo "<tr>"; ?>
	        				<?php
									echo "<td style='font-weight:bold;'>".$key1."</td>";
	        				foreach ($l_keba as $key2 => $value2) { ?>
	        				<td><!-- <?php echo $key2."/".$key1; ?> -->
								<?php echo number_format(($value2/$value1),2); ?>
							</td>
							<?php }
								echo "<td>".number_format($pv_keba[$key1],4)."</td></tr>";
								$total = $total + $pv_keba[$key1];
							}
 							?>
 							<tr class="success">
							<td style='font-weight:bold;'>Jumlah</td>
						<?php 
						foreach ($jml_keba as $key => $value) {
	        ...
	        <a href="<?php echo base_url(); ?>dashboard/kalkulasi/4" class="btn btn-inverse" style="float:right;">next</a>
