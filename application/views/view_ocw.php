...
<html lang="en">
    ...
    <div class="kalkulasi-content"> <!-- start main content -->
        ...
        <div class="fordatagrid"> <!-- start span11 -->
        	<?php 
        	 ...
        	 <table class="table table-stripped table-bordered table-condensed dtgrid">
				<thead>
					<td colspan="2"></td>
					<?php
					foreach ($pv as $key1 => $value1) {
						$list = $key1;
					}
					foreach ($pv[$list] as $key => $value) {
						echo "<td>".$key."</td>";
					}
					?>
				</thead>
			...
			<?php
				foreach ($pv_modal as $key => $value) {
					...
					foreach ($pv[$key] as $key1 => $value1) {
						...
						echo "<td>". number_format($value1,4). "</td>";	
					}
					echo "</tr>";
				}
				echo "<tr>";
			...
			<a href="<?php echo base_url() ?>dashboard/kalkulasi/6" class="btn btn-inverse" style="float:right;">next</a>
