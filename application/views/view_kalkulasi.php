...
<html lang="en">
    ...
    <div class="kalkulasi-content"> <!-- start main content -->
      <div class="container"> <!-- start container -->
        <div class="span6 fordatagrid"> <!-- start span11 -->
        	<?php 
        		if(!empty($l_modal) && !empty($jml_modal) && !empty($pv_modal)) {	?>
	        <table class="table table-stripped table-bordered table-condensed dtgrid">
	        	...
	        	<tbody>
	        		<?php 
	        			foreach ($l_modal as $key1 => $value1) {
	        				echo "<tr>";
									echo "<td style='font-weight:bold;'>".$key1."</td>";
	        				foreach ($l_modal as $key2 => $value2) { ?>
	        		<td>
								<?php echo number_format(($value2/$value1),2); ?>
							</td>
							<?php }
								echo "<td>".number_format($pv_modal[$key1],4)."</td></tr>";
								$total = $total + $pv_modal[$key1];
							}
 							?>
 							<tr class="success">
							<td style='font-weight:bold;'>Jumlah</td>
						<?php 
						foreach ($jml_modal as $key => $value) {
	        ...
	        <a href="<?php echo base_url(); ?>dashboard/kalkulasi/2" class="btn btn-inverse" style="float:right;">next</a>
